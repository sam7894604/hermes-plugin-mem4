...
1. **Auditor** (`memory.mem4.audit.enabled: true`) — one JSONL line per recall/route/prefetch event to `$HERMES_HOME/mem4/audit.jsonl` (query, hit/miss, route `fts`/`trigram`/`like`, injected chars). A tool-call *miss* is **precise**; the L0-hit rate (turns using no tool) is **estimated** offline and labelled as such.
